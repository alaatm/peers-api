...
public sealed class MashkoorContextTests
{
    ...
    {
        var aggregateRoots = from t in typeof(MashkoorContext).Assembly.GetTypes()
                             where typeof(IAggregateRoot).IsAssignableFrom(t) && !typeof(IAggregateRoot).IsAssignableFrom(t.BaseType)
                             select t;

        var definedDbSets = from e in typeof(MashkoorContext).GetProperties(BindingFlags.Instance | BindingFlags.Public)
    ...
    public void Migrates_with_no_exceptions()
    {
        ...
        var options = new DbContextOptionsBuilder<MashkoorContext>().UseSqlServer(_connStr).Options;
        using var context = new MashkoorContext(options);
