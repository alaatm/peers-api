...
namespace Mashkoor.Modules.Test.Kernel.Pipelines;
...
public sealed class IdentityCheckBehaviorTests : IDisposable
{
    ...
    {
        // Arrange
        var context = InMemoryMashkoorContext.Create();
    ...
    public async Task Executes_next_when_no_auth_attribute_and_non_authenticated_user()
    {
        ...
        var nextCalled = false;
        var handler = new IdentityCheckBehavior<TestCommandNoAuth, IResult>(
            BuildServiceProvider(InMemoryMashkoorContext.Create()),
            identityMoq.Object,
            new LoggerNoAuth(),
            new SLMoq<res>());
    ...
    private static ServiceProvider BuildServiceProvider(MashkoorContext context)
