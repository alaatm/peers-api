...
public abstract partial class IntegrationTestBase
{
  ...
  ""client_x509_cert_url"": ""https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-xxxxx%40test.iam.gserviceaccount.com""
    ...
    public void ExecuteDbContext(Action<PeersContext> action)
        => ExecuteScope(sp => action(sp.GetService<PeersContext>()));

    public T ExecuteDbContext<T>(Func<PeersContext, T> action)
        => ExecuteScope(sp => action(sp.GetService<PeersContext>()));

    public Task ExecuteDbContextAsync(Func<PeersContext, Task> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<PeersContext>()));

    public Task<T> ExecuteDbContextAsync<T>(Func<PeersContext, Task<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<PeersContext>()));

    public Task ExecuteDbContextAsync(Func<PeersContext, IMediator, Task> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<PeersContext>(), sp.GetService<IMediator>()));

    public Task<T> ExecuteDbContextAsync<T>(Func<PeersContext, IMediator, Task<T>> action)
        => ExecuteScopeAsync(sp => action(sp.GetService<PeersContext>(), sp.GetService<IMediator>()));
    ...
    public async Task<AppUser> InsertManagerAsync(string email = null, string password = null, params string[] roles)
    {
        ...
        return await ExecuteScopeAsync(async sp =>
        {
            ...
            var um = sp.GetRequiredService<IdentityUserManager<AppUser, PeersContext>>();
    ...
    {
        var um = Services.GetRequiredService<IdentityUserManager<AppUser, PeersContext>>();
