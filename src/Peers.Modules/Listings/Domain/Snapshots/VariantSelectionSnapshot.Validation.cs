...
public partial record VariantSelectionSnapshot
{
    ...
    {
        if (!string.Equals(SnapshotId, ctx.Snapshot.SnapshotId, StringComparison.Ordinal))
        {
            throw StateError($"SnapshotId does not match listing's snapshot id '{ctx.Snapshot.SnapshotId}'.");
        }

        // Must contain exactly one ref per axis in schema
        if (Selections.Count != ctx.AxesSnapshot.Count)
        {
            throw StateError($"Selection refs count '{Selections.Count}' does not match axes count '{ctx.AxesSnapshot.Count}'.");
        }
        ...
        foreach (var sel in Selections)
        {
            ...
            {
                throw StateError($"Selection '{sel}' references unknown axis.");
            }
